...
@section('content')
    ...
    <div class="container">
        <form action="{{route('update-post')}}" method="post" class="creator" enctype="multipart/form-data">
            <h1>A new post</h1>
            @csrf
            ...
            <div class="meta-field">
                ...
                <div class="row">
                    <div class="minimo-text-input">
                        <textarea required type="textarea" rows="3" name="short">{{ $post->short }}</textarea>
                        <span>Short</span>
                    </div>
                </div>
                <div class="row">
                    <div class="minimo-file-input">
                        <input type="file" class="creator-meta-image" name="post-picture" hidden="hidden" accept="image/*">
                        <button type="button" class="minimo-button" default="{{ $post->image }}">{{ $post->image }}</button>
                    </div>
                </div>
            </div>
            <div class="creator-toolbar">
                ...
                <iframe class="creator-field" id="newTextArea" name="newTextArea"></iframe>
            </div>
    ...
    <script>
        window.onload = () => {
            ...
            newTextArea.document.body.innerHTML = `{!! $post->content !!}`;
        };
